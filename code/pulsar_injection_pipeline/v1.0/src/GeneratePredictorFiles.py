...
class GeneratePredictorFiles:
    ...
    def main(self, argv=None):
        ...
        # REQUIRED ARGUMENTS
        parser.add_option("-p", action="store", dest="parDir",
        ...
        parser.add_option("-s", type="int", dest="secs",
                          help='The total of seconds to pass in to tempo2 (optional).', default=600)

        parser.add_option("-b", type="int", dest="batch",
                          help='The number of files to process in a batch (optional).', default=1000)

        parser.add_option("--f1", type="int", dest="f1",
                          help='The frequency of the first channel passed in to tempo2 (optional).', default=1350)

        parser.add_option("--f2", type="int", dest="f2",
                          help='The frequency of the last channel passed in to tempo2 (optional).', default=1670)

        parser.add_option("--tcoeff", type="int", dest="tcoeff",
                          help='The number of time coefficients to be computed by tempo2 (optional).', default=12)

        parser.add_option("--fcoeff", type="int", dest="fcoeff",
                          help='The number of frequency coefficients to be computed by tempo2 (optional).', default=2)

        parser.add_option("--mjd1", action="store", dest="mjd1",
                          help='Start time MJD.', default="56000")

        parser.add_option("--mjd2", action="store", dest="mjd2",
                          help='Start time MJD.', default="56001")

        parser.add_option("--tel", action="store", dest="tel",
        ...
        print("\tPar directory path: "     + str(self.parDir))
        print("\tOutput directory path: "  + str(self.outputDir))
        print("\tSegment length: "         + str(self.obsLength))
        print("\tF1 value: "               + str(self.f1))
        print("\tF2 value: "               + str(self.f2))
        print("\tNumber of time coeffs: "  + str(self.tcoeff))
        print("\tNumber of freq. coeffs: " + str(self.fcoeff))
        print("\tMJD 1: "                  + str(self.mjd1))
        print("\tMJD 2: "                  + str(self.mjd2))
        print("\tBatch size: "             + str(self.batch))
        ...
        if self.tcoeff <= 0:
            print("\n\tSupplied number of time coefficients invalid - Exiting!")
            sys.exit()
        ...
        if os.path.isdir(self.outputDir) is False:
            ...
            if os.path.exists(self.pulsarPredictorFileDir) is False:
                os.makedirs(self.pulsarPredictorFileDir)

            if os.path.exists(self.fakePulsarPredictorFileDir) is False:
            ...
            for filename in filenames:
                ...
                for ext in self.extensions:
                    ...
                    if ext in path:
                        ...
                        print("\tPath: " + str(path) + " Filename: " + str(name))
                        ...
                        if batchEntryCount < self.batch:
                            ...
                            tempo2Command = "tempo2 -f " + path + " -pred \"" + self.telescope + " " + self.mjd1 + " " +\
                                            self.mjd2 + " " + str(self.f1) + " " + str(self.f2) + " " +\
                                            str(self.tcoeff) + " " + str(self.fcoeff) + " " + str(self.obsLength)+"\""
                            ...
                            if os.path.exists("t2pred.dat"):
                                ...
                                if "FakePulsar_" in name:
                                    ...
                                    if os.path.exists(fakePulsarDestPath):
                                    ...
                                    # Check the file exists.
                                    if os.path.exists(pulsarDestPath):
                                        # The file was copied successfully.
                                ...
                                print("\tError generating predictor file for par: " + str(path))
